...
MI_BOOL g_osd_dilate_bmp;
MI_BOOL g_osd_dilate_profiling;

MI_U32 Font_Get_StrLen(const MI_S8 *ps8String)
{
    MI_U8 *u8Str;
    MI_U8 u8Count = 0;
    MI_U32 u32Len = 0;
    ...
    u8Str = (MI_U8 *)ps8String;
    u32Len = strlen((char *)ps8String);
    
    while((u32Len > 0)&& u8Str)
    {
        if((*u8Str) < 0x80)
        {
            u8Count++;
...
MI_U32 Font_Strcmp(MI_S8* pStr1, MI_S8* pStr2)
{
    ...
    hzLen = Font_Get_StrLen(pStr1);

    for(idx = 0; idx < hzLen; idx ++)
    {
        offset_1 = Font_GetCharQW(pStr1, &qu1, &wei1);
        offset_2 = Font_GetCharQW(pStr2, &qu2, &wei2);
...
static void Font_Close(MI_Font_t* pFont)
{
...
static void Font_Open(OsdFontSize_e eFontSize)
{
    ...
    if((FONT_SIZE_8 == eFontSize)||/*(FONT_SIZE_16 == eFontSize)||*/
       (FONT_SIZE_40 == eFontSize)||(FONT_SIZE_56 == eFontSize)||(FONT_SIZE_80 == eFontSize))
    {
        sprintf((char *)hz_path, "%s/font/HZ8.mz", g_u8OsdFontPath);
        ...
        font_size = 8;
            ...
            (FONT_SIZE_48 == eFontSize)||(FONT_SIZE_60 == eFontSize)||(FONT_SIZE_72 == eFontSize)||(FONT_SIZE_84 == eFontSize))
    {
        sprintf((char *)hz_path, "%s/font/HZ12.mz", g_u8OsdFontPath);
        bin_size = HZ_12P_BIN_SIZE;
        font_size = 12;
    }
    else if((FONT_SIZE_16 == eFontSize)||(FONT_SIZE_32 == eFontSize)||(FONT_SIZE_64 == eFontSize)||(FONT_SIZE_96 == eFontSize))
    {
        sprintf((char *)hz_path, "%s/font/HZ16.mz", g_u8OsdFontPath);
...
void YuvDilate(MI_U8* pYbuffer, MI_U16* pUvbuffer, MI_U32 stride, MI_U32 width, MI_U32 height, YUVColor_t* pYuvColor)
{
    MI_U32 i, j;
    ...
    if(g_osd_dilate_profiling)
    {
        ...
        if((MI_U32)end.tv_usec > dilate_max_time)
        {
            ...
            printf("dilate_max_time:%u usec\n", dilate_max_time);
        }
    }
}

void BmpDilate(MI_U16* pbuffer , MI_U32 width , MI_U32 height , MI_U32 stride , MI_U16 rgbColor)
{
    MI_U32 i, j;
    struct timeval start, end;
    MI_U16* pRGBbuffer;
                           ...
                           MI_U32 nIndex, YUVColor_t* pfColor, YUVColor_t* pbColor , MI_BOOL bOutline)
{
    ...
    if(bOutline)
    {
        YuvDilate(yBuffer, (MI_U16*)uvBuffer, pYuv->width, nMultiple * pFont->nFontSize, nMultiple * pFont->nFontSize, pfColor);
    }
                            ...
                            MI_U32 nIndex, Color_t* pfColor, Color_t* pbColor, MI_BOOL bOutline)
{
    ...
    if(bOutline)
    {
        BmpDilate((MI_U16*)pBuffer, nMultiple * pFont->nFontSize, nMultiple * pFont->nFontSize, pBmp->width, fcolor4444);
    }
                            ...
                            MI_U32 nIndex, Color_t* pfColor, Color_t* pbColor, MI_BOOL bOutline)
{
    ...
    if(bOutline)
    {
        BmpDilate((MI_U16*)pBuffer, nMultiple * pFont->nFontSize, nMultiple * pFont->nFontSize, pBmp->width, fcolor1555);
    }

    return MI_SUCCESS;
}

MI_S32 Font_DrawPaletteTable_I4(MI_Font_t* pFont , MI_U32 qu , MI_U32 wei, MI_U32 nMultiple, ImageData_t* pBmp,
                                       MI_U32 nIndex, Color_t* pfColor, Color_t* pbColor, MI_BOOL bOutline)
{
    MI_U32 nCharWidth = (pFont->nFontSize + 7) / 8;
    MI_U8* pFontBitmap = pFont->pData + ((qu - 1) * 94 + wei - 1) * nCharWidth * pFont->nFontSize;
                         ...
                         MI_U32 nSpace, Color_t* pfColor, Color_t* pbColor, MI_BOOL bOutline)
{
   ...
   MI_U32 hzLen = Font_Get_StrLen(pStr);

   if(NULL == (pFont = Font_GetHandle(eFontSize, &nMultiple)))
   {
   ...
   for(idx = 0; idx < hzLen; idx++)
   {
       offset = Font_GetCharQW(pStr, &qu, &wei);
       pStr += offset;
       ...
       switch(pImage->pmt)
       {
        /*   case COLOR_FormatYUV420SemiPlanar:
           ...
           case E_MI_RGN_PIXEL_FORMAT_ARGB4444:
               Font_DrawRGB4444(pFont, qu, wei, nMultiple, pImage, idx, pfColor, pbColor, bOutline);
               break;

           case E_MI_RGN_PIXEL_FORMAT_ARGB1555:
               Font_DrawRGB1555(pFont, qu, wei, nMultiple, pImage, idx, pfColor, pbColor, bOutline);
               break;

           case E_MI_RGN_PIXEL_FORMAT_I4:
               Font_DrawPaletteTable_I4(pFont, qu, wei, nMultiple, pImage, idx, pfColor, pbColor, bOutline);
               break;
...
{
    memset((char*)g_u8OsdFontPath, 0x00, sizeof(g_u8OsdFontPath));
	memcpy((char*)g_u8OsdFontPath, OsdFontPath, MIN(sizeof(g_u8OsdFontPath), strlen(OsdFontPath)));
}
